...
lst = [] 
for line in fopen:
# split the lin einto a list words
    words = line.split()
# for each word check to see if word exists
    for word in words:
# if the word is not on the list then add it
        if word not in lst:
            lst.append(word)
# when completed sort the list
print(sorted(lst),'new')